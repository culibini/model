...
@njit(cache=True)
def generate_candidates_det(cx0, cy0, radius, width, height,
...
def kernel(start_x, start_y, goal_x, goal_y, start_level, goal_level,
           ...
           hour_switch_penalty, wastar, k_labels):
    ...
    front_idx = np.zeros(n4 * K, dtype=np.uint32)
    front_n = np.zeros(n4, dtype=np.uint8)

    lcap = 1 << 16
    lab_cost = np.empty(lcap, dtype=np.float64)
    lab_tdist = np.empty(lcap, dtype=np.float64)
    lab_stay = np.empty(lcap, dtype=np.float64)
    lab_hstay = np.empty(lcap, dtype=np.float64)
    lab_state = np.empty(lcap, dtype=np.uint32)
    lab_prev = np.empty(lcap, dtype=np.uint32)

    hcap = 1 << 16
    hp = np.empty(hcap, dtype=np.float64)
    hlab = np.empty(hcap, dtype=np.uint32)
    hs = 0
    ...
    lab_stay[0] = 0.0
    lab_hstay[0] = 0.0
    lab_state[0] = np.uint32(start_ni)
    lab_prev[0] = NO_PREV
    ln = 1
    front_idx[start_ni * K] = np.uint32(0)
    front_n[start_ni] = 1
    hp[0] = 0.0
    hlab[0] = np.uint32(0)
    hs = 1
    ...
    while hs > 0 and nodes_processed < max_nodes:
        if goal_found and hp[0] > goal_cost:
            break
        cur_li = np.int64(hlab[0])
        hs -= 1
        hp[0] = hp[hs]
        hlab[0] = hlab[hs]
        i = 0
        while True:
            ...
            if sm == i:
                break
            hp[i], hp[sm] = hp[sm], hp[i]
            hlab[i], hlab[sm] = hlab[sm], hlab[i]
            i = sm

        cur_ni = np.int64(lab_state[cur_li])
        fbase0 = cur_ni * K
        alive = False
        for j in range(int(front_n[cur_ni])):
            if front_idx[fbase0 + j] == np.uint32(cur_li):
                alive = True
                break
        if not alive:
            continue
        ...
        cur_cost = lab_cost[cur_li]

        if (abs(real_x - goal_x) <= goal_tolerance and
                abs(real_y - goal_y) <= goal_tolerance and level == goal_level):
            if (not goal_found or cur_cost < goal_cost or
                    (cur_cost == goal_cost and lab_tdist[cur_li] < goal_td)):
                goal_found = True
                goal_cost = cur_cost
                goal_td = lab_tdist[cur_li]
                goal_li = cur_li
            if not collect_ties:
                break
            continue

        current_total_distance = lab_tdist[cur_li]
        current_stay = lab_stay[cur_li]
        current_hour_stay = lab_hstay[cur_li]
        ...
        for k in range(nb_cnt):
            ...
            for lvl_shift in range(lvl_lo, lvl_hi + 1):
                ...
                for hour_shift in range(hs_begin, hs_end + 1):
                    ...
                    if intentional_shift > 0:
                        ...
                        if need_extra > 0.0 and nb_ok:
                            ...
                            while rem > 1e-9:
                                ...
                                rem -= chunk
                            added_cost = add_c
                            ntd_out = new_total_distance + need_extra
                    ...
                    dominated = False
                    for j in range(fn):
                        li0 = np.int64(front_idx[fbase + j])
                        if (lab_cost[li0] <= new_cost and
                                lab_tdist[li0] <= ntd_out and
                                lab_stay[li0] <= stay_out and
                                lab_hstay[li0] <= new_hour_stay):
                            dominated = True
                            break
                    ...
                    if not dominated:
                        ...
                        for j in range(fn):
                            li0 = np.int64(front_idx[fbase + j])
                            if (new_cost <= lab_cost[li0] and
                                    ntd_out <= lab_tdist[li0] and
                                    stay_out <= lab_stay[li0] and
                                    new_hour_stay <= lab_hstay[li0]):
                                continue
                            front_idx[fbase + wpos] = np.uint32(li0)
                            wpos += 1
                        insert_ok = True
                        if wpos == K:
                            mj = 0
                            ...
                            for j in range(K):
                                ...
                                if lab_cost[li0] > mc:
                                    ...
                                    mj = j
                            if new_cost < mc:
                                front_idx[fbase + mj] = front_idx[fbase + K - 1]
                                wpos = K - 1
                            else:
                                insert_ok = False
                        if insert_ok:
                            if ln == lcap:
                                lcap *= 2
                                t1 = np.empty(lcap, dtype=np.float64)
                                t1[:ln] = lab_cost[:ln]
                                lab_cost = t1
                                t2 = np.empty(lcap, dtype=np.float64)
                                t2[:ln] = lab_tdist[:ln]
                                lab_tdist = t2
                                t3 = np.empty(lcap, dtype=np.float64)
                                t3[:ln] = lab_stay[:ln]
                                lab_stay = t3
                                t4 = np.empty(lcap, dtype=np.float64)
                                t4[:ln] = lab_hstay[:ln]
                                lab_hstay = t4
                                t5 = np.empty(lcap, dtype=np.uint32)
                                t5[:ln] = lab_state[:ln]
                                lab_state = t5
                                t6 = np.empty(lcap, dtype=np.uint32)
                                t6[:ln] = lab_prev[:ln]
                                lab_prev = t6
                            lab_cost[ln] = new_cost
                            lab_tdist[ln] = ntd_out
                            lab_stay[ln] = stay_out
                            lab_hstay[ln] = new_hour_stay
                            lab_state[ln] = np.uint32(ni)
                            lab_prev[ln] = np.uint32(cur_li)
                            front_idx[fbase + wpos] = np.uint32(ln)
                            front_n[ni] = wpos + 1

                            if hs == hcap:
                                hcap *= 2
                                nhp = np.empty(hcap, dtype=np.float64)
                                nhl = np.empty(hcap, dtype=np.uint32)
                                nhp[:hs] = hp[:hs]
                                nhl[:hs] = hlab[:hs]
                                hp = nhp
                                hlab = nhl
    ...
    if not goal_found:
        ...
        for gy2 in range(grid_h):
            for gx2 in range(grid_w):
                ...
                if not (abs(rx2 - goal_x) <= goal_tolerance * 3 and
                        abs(ry2 - goal_y) <= goal_tolerance * 3):
                    continue
                cell2 = gy2 * grid_w + gx2
                for hr2 in range(max_hours):
                    ni2 = (cell2 * num_levels + goal_level) * max_hours + hr2
                    for j in range(int(front_n[ni2])):
                        li0 = np.int64(front_idx[ni2 * K + j])
                        c2 = lab_cost[li0] + hgrid[cell2]
                        t2v = lab_tdist[li0]
                        if c2 < best_cost or (c2 == best_cost and t2v < best_td2):
                            best_cost = c2
                            best_td2 = t2v
                            goal_li = li0
        if best_cost < inf:
    ...
    pos = cnt - 1
    while True:
        st = np.int64(lab_state[li])
        hr = int(st % max_hours)
        lv = int((st // max_hours) % num_levels)
        cc = st // (max_hours * num_levels)
        gxx = int(cc % grid_w)
        gyy = int(cc // grid_w)
        ppx[pos] = np.float64(gxx * step_size)
        ppy[pos] = np.float64(gyy * step_size)
        plvl[pos] = lv
        pstay[pos] = lab_stay[li]
        pdist[pos] = lab_tdist[li]
        parr[pos] = hr
        pos -= 1
        pr = lab_prev[li]
        if pr == NO_PREV:
...
def optimize_stage2(xs, ys, levels, stay, total, arrs, frozen, w_safety, w_length,
    ...
    for _ in range(NUM_ITERATIONS):
        ...
        for i in range(1, n_points - 1):
            ...
            gx_arr, gy_arr = generate_candidates_det(
...
class Session:
    def __init__(self, danger_map, forecasts, threads=0):
        if threads > 0:
            set_num_threads(threads)

        self.danger2d = np.ascontiguousarray(danger_map, dtype=np.float64).copy()
        apply_hyperbolic(self.danger2d)
        self.fc4 = np.ascontiguousarray(forecasts, dtype=np.float64).copy()
        apply_hyperbolic(self.fc4)

        self.height, self.width = self.danger2d.shape
        self.n_hours = self.fc4.shape[0]
        self.scale_y = float(self.height) / float(self.fc4.shape[2])
        self.scale_x = float(self.width) / float(self.fc4.shape[3])

        self.step = DIJKSTRA_STEP_SIZE
        self.grid_w = int(np.ceil(float(self.width) / self.step)) + 1
        self.grid_h = int(np.ceil(float(self.height) / self.step)) + 1

        self.dg = build_danger_grid(self.danger2d, self.fc4, LEVEL_PENALTIES,
                                    self.grid_w, self.grid_h, self.step,
                                    NUM_LEVELS, self.n_hours, self.width, self.height,
                                    self.scale_x, self.scale_y)
        self.mn, self.has_nb = build_min_nb(self.dg, self.grid_w, self.grid_h, self.step,
                                            self.width, self.height, NUM_LEVELS,
                                            self.n_hours)
        self.md = build_min_dh(self.dg, self.grid_w, self.grid_h, NUM_LEVELS,
                               self.n_hours)

    def route(self, points, wastar=1.0):
        ...
        for si in range(len(route_xy) - 1):
            ...
            if route_levels is not None:
                ...
                goal_level = -1
            if start_level < 0:
                start_level = find_best_level(sx, sy, 0.0, danger2d, fc4,
                                              scale_x, scale_y,
                                              dk_dang, dk_hour, order, ostate)
            if goal_level < 0:
            ...
            hg = build_hgrid(self.md, grid_w, grid_h, step, width, height,
            ...
            (kx, ky, klv, kst, ktd, kar, ok, truncated) = kernel(
                sx, sy, ex, ey, start_level, goal_level, danger2d, self.dg,
                self.mn, self.has_nb, hg,
                ...
                HOUR_STAY_DISTANCE, HOUR_SWITCH_PENALTY, w_eff, K_LABELS)
            ...
            mx, my, mlv, mst, mtd, mar, mfz = materialize_loiter_loops(
                ...
                self.dg, grid_w, grid_h, width, height, step, NUM_LEVELS,
                n_hours, fs_scaled)
            ...
            for i in range(len(rx) - 1):
                d_length += float(np.sqrt((rx[i + 1] - rx[i]) ** 2 +
                                          (ry[i + 1] - ry[i]) ** 2))

            safety = SAFETY_WEIGHT * (1.0 + SAFETY_WEIGHT_COEFF *
                                      (d_length / BASE_PATH_LENGTH))
...
def calculate(danger_map, forecasts, points, wastar=1.0, threads=0):
    return Session(danger_map, forecasts, threads).route(points, wastar)
