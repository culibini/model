...
from .config import (BASE_PATH_LENGTH, DIJKSTRA_DANGER_WEIGHT,
                     ...
                     SAFETY_WEIGHT_COEFF)
from .danger import DangerCache, apply_hyperbolic
from .geometry import materialize_loiter_loops, refine_path
from .optimize import optimize_stage2
from .precompute import (build_danger_grid, build_hgrid, build_min_dh,
                         build_min_nb, set_threads)
from .rng import RNG
...
def calculate(danger_map, forecasts, points, wastar=1.0, threads=0, seed=RNG_SEED):
    ...
    rng = RNG(seed)
    cache = DangerCache(danger2d, fc4, scale_x, scale_y)
    ...
    for si in range(len(route_xy) - 1):
        ...
        if route_levels is not None:
            ...
            goal_level = -1
        if start_level < 0:
            start_level = cache.find_best_level(sx, sy, 0.0)
        if goal_level < 0:
            straight = float(np.sqrt((ex - sx) ** 2 + (ey - sy) ** 2))
            goal_level = cache.find_best_level(ex, ey, straight)
